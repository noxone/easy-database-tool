 ...
 * <code>change()</code> method is called to report the position and length of a
 * change in the text, and the <code>scan()</code> method is called to perform
 * scanning or rescanning. For example, to scan an entire document held in a
 * character array <code>text</code> in one go: <blockquote>
 * 
 * <pre>
 ...
 * </blockquote>
 * <p>
 * For incremental scanning, the <code>position()</code> method is used to find
 * the text position at which rescanning should start. For example, a syntax
 * highlighter might contain this code: <blockquote>
 * 
 * <pre>
...
public class Scanner implements TokenTypes
{
	 ...
	 * left after the last call to read, or the type of the old token at the
	 * same position if rescanning, or WHITESPACE if at the start of a document.
	 * The method succeeds in all cases, returning whitespace or comment or
	 * error tokens where necessary. Each line of a multi-line comment is
	 * treated as a separate token, to improve incremental rescanning. If the
	 * buffer does not extend to the end of the document, the last token
	 * returned for the buffer may be incomplete and the caller must rescan it.
	 * The read method can be overridden to implement different languages. The
	 * default version splits plain text into words, numbers and punctuation.
	 */
	protected int read()
	{
		char c = buffer[start];
	...
	/**
	 * Find out at what text position any remaining scanning work should start,
	 * or -1 if scanning is complete.
	 */
	public int position()
	{
		if (!scanning)
